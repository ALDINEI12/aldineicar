...
function toggleResumoFinanceiro() {
  var box = document.getElementById('dashDetalhesBox') || document.querySelector('.dash-detalhes');
  if (!box) return;
  var aberto = box.getAttribute('data-aberto') === '1';
  if (aberto) {
    box.style.display = 'none';
    box.setAttribute('data-aberto', '0');
    var btn = document.getElementById('btnToggleResumoFin');
    if (btn) btn.classList.remove('ativo');
  } else {
    box.style.display = 'block';
    box.setAttribute('data-aberto', '1');
    var btn2 = document.getElementById('btnToggleResumoFin');
    if (btn2) btn2.classList.add('ativo');
    if (typeof renderDashboard === 'function') renderDashboard();
    setTimeout(function() {
...
function abrirResumoFinanceiroToast() {
  try {
    ...
    if (ini && fim && typeof formatarDataBR === 'function') {
      periodoTxt = formatarDataBR(ini) + ' → ' + formatarDataBR(fim);
    } else if (typeof __dashPeriodoRapido !== 'undefined') {
      var map = { hoje: 'Hoje', ontem: 'Ontem', '7dias': 'Últimos 7 dias', mes: 'Este mês', ano: 'Este ano', tudo: 'Todo o período' };
      periodoTxt = map[__dashPeriodoRapido] || __dashPeriodoRapido;
    }
    var msg = 'Resumo financeiro' + (periodoTxt ? ' (' + periodoTxt + ')' : '')
      + ' · Faturamento: ' + fmt(fat)
      + ' · Materiais: ' + fmt(mat)
      + ' · Gastos: ' + fmt(gastos)
      + ' · Lucro: ' + fmt(lucro);
    if (typeof mostrarToast === 'function') mostrarToast(msg, 'sucesso');
    else alert(msg);
    var box = document.getElementById('dashDetalhesBox') || document.querySelector('.dash-detalhes');
    if (box && box.getAttribute('data-aberto') !== '1') toggleResumoFinanceiro();
    else if (box) { try { box.scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e2) {} }
  } catch (e) {
    console.warn(e);
    if (typeof mostrarToast === 'function') mostrarToast('Não foi possível montar o resumo.', 'erro');
  }
}
