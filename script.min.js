...
async function editarProduto(id) {
 ...
 else if (mostrarIdx.length < totalHist) meta.textContent = 'Mostrando ' + mostrarIdx.length + ' de ' + totalHist + ' serviços.';
 else meta.textContent = totalHist + ' serviço(s).';
 }
 if (totalHist === 0) {
...
function ativarMenuAdminSePermitido(email) {
  ...
  // também compara direto (fallback)
  if (!ok && email) {
    var e = String(email).trim().toLowerCase();
    if (e === 'nbdynei707@gmail.com') ok = true;
  }
  var btn = document.getElementById('btn-admin-nav');
  var side = document.getElementById('btn-admin-side');
  if (btn) {
    btn.style.display = ok ? 'flex' : 'none';
    btn.style.visibility = ok ? 'visible' : 'hidden';
  }
  if (side) {
    side.style.display = ok ? '' : 'none';
  }
  window.__isAdmin = !!ok;
...
function aplicarMenuSomenteAdmin(ativo) {
  try {
    ...
    document.querySelectorAll('.sidebar button').forEach(function(btn) {
      ...
      if (id === 'btn-admin-side' || oc.indexOf("admin") >= 0) {
    ...
    if (sideAdm) sideAdm.style.display = ativo ? '' : 'none';
  } catch (e) {
    console.warn(e);
  }
}
...
(function() {
  ...
  window.ativarMenuAdminSePermitido = function(email) {
    ...
    var ok = typeof isEmailAdmin === 'function' && isEmailAdmin(email);
    if (!ok && email) ok = String(email).toLowerCase() === 'nbdynei707@gmail.com';
    aplicarMenuSomenteAdmin(!!ok);
    if (ok && typeof mostrarAba === 'function') {
      setTimeout(function() {
        try { mostrarAba('admin'); } catch (e) {}
      }, 400);
    }
  };
})();
