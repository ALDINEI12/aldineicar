...
async function carregarPainelAdmin() {
  ...
  try {
    ...
    var json = await res.json().catch(function() { return {}; });
    if (!res.ok) throw new Error(json.error || json.message || ('HTTP ' + res.status));
    var lista = json.oficinas || [];
    var meta = '';
    if (json.total_auth != null) meta += 'Contas Auth: ' + json.total_auth + ' · ';
    if (json.total_user_data != null) meta += 'Com dados: ' + json.total_user_data + ' · ';
    if (json.user_data_error) meta += 'Erro user_data: ' + json.user_data_error + ' · ';
    if (!lista.length) {
      box.innerHTML = '<p style="padding:12px;color:#64748b;">Nenhuma oficina encontrada.</p>'
        + (meta ? '<p style="padding:0 12px 12px;font-size:12px;color:#94a3b8;">' + meta + '</p>' : '')
        + '<p style="padding:0 12px 12px;font-size:12px;color:#94a3b8;">Se houver contas no Authentication e aqui continua vazio, a function ainda está na versão antiga — faça deploy de novo.</p>';
      return;
    }
    box.innerHTML = '<p style="font-size:13px;color:#64748b;margin-bottom:10px;">' + lista.length + ' conta(s)</p>'
      + (meta ? '<p style="font-size:12px;color:#94a3b8;margin:-4px 0 10px;">' + meta + '</p>' : '')
      ...
      return '<div class="admin-oficina-card' + (bloq ? ' admin-oficina-bloqueada' : '') + '">'
        + '<div class="admin-oficina-head"><strong>' + (o.nome || '—') + '</strong>' + badge + '</div>'
        ...
        + '<div class="admin-oficina-acoes">' + btn + '</div>'
        + '</div>';
    }).join('');
  } catch (e) {
    console.error(e);
    box.innerHTML = '<p style="color:#dc2626;padding:12px;">Erro admin: ' + (e.message || e) + '</p>'
      + '<p style="color:#64748b;font-size:12px;padding:0 12px 12px;">Confira se a Edge Function <b>admin-listar-oficinas</b> está no ar e o secret <b>ADMIN_EMAILS=nbdynei707@gmail.com</b>.</p>';
  }
}
...
async function adminAcaoConta(userId, action) {
  ...
  var msg = action === 'bloquear'
    ? 'Bloquear esta conta? Ela não conseguirá entrar no sistema.'
  ...
  try {
    ...
    if (typeof mostrarToast === 'function') {
      mostrarToast(action === 'bloquear' ? 'Conta bloqueada.' : 'Conta desbloqueada.', 'sucesso');
    } else {
      alert(action === 'bloquear' ? 'Conta bloqueada.' : 'Conta desbloqueada.');
    }
