...
async function uploadFotoOficina(dataUrl, pasta) {
 ...
 // Garante URL absoluta do projeto
 if (url.indexOf('http') !== 0 && typeof supabaseUrl === 'string') {
 url = String(supabaseUrl).replace(/\/$/, '') + '/storage/v1/object/public/' + STORAGE_BUCKET_FOTOS + '/' + pathFinal;
 }

 // cache-bust leve na 1ª exibição (ajuda CDN/SW)
 if (url.indexOf('?') < 0) url = url + '?v=' + Date.now();
...
function renderServicosVitrineAdmin() {
 ...
 var foto = s.foto || s.imagem || '';
 var img = foto
 ? '<div class="servico-admin-thumb-wrap"><img class="servico-admin-thumb" src="'+foto+'" alt="" onerror="window.__retryFoto&&window.__retryFoto(this)"></div>'
...
function normalizarUrlFoto(u) {
  ...
  // remove cache-bust antigo se precisar reaplicar
  return u;
}
...
function urlFotoValida(u) {
  ...
  if (!u) return false;
  if (u.indexOf('data:image') === 0) return u.length > 64;
  if (/^https?:\/\//i.test(u)) return true;
  return false;
}

/** Corrige URLs relativas do Storage e força https do projeto */
function resolverUrlFoto(u) {
  u = normalizarUrlFoto(u);
  if (!u) return '';
  if (u.indexOf('data:image') === 0) return u;
  if (/^https?:\/\//i.test(u)) {
    // se for signed URL expirada comum, ainda tenta
    return u;
  }
  // path só: userId/pasta/file.jpg
  if (u.indexOf('fotos-oficina') < 0 && u.indexOf('/') >= 0 && typeof supabaseUrl === 'string') {
    return String(supabaseUrl).replace(/\/$/, '') + '/storage/v1/object/public/fotos-oficina/' + u.replace(/^\//, '');
  }
  return u;
}
...
function htmlImgProduto(url, dark) {
  ...
  return '<img src="' + safe + '" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer" style="width:100%;height:110px;object-fit:cover;border-radius:8px;margin-bottom:8px;background:' + bg + ';" onerror="window.__retryFoto&&window.__retryFoto(this)">';
}
...
function htmlImgServicoPublico(url, nomeEsc) {
  ...
  var safe = String(src).replace(/&/g, '&amp;').replace(/"/g, '&quot;');
  return '<div class="vt-servico-foto"><img src="' + safe + '" alt="' + (nomeEsc || '') + '" loading="lazy" referrerpolicy="no-referrer" onerror="window.__retryFoto&&window.__retryFoto(this)"></div>';
}

/** Se a foto falhar, tenta 1x sem querystring e 1x com cache-bust; só então placeholder */
window.__retryFoto = function(img) {
  try {
    if (!img || img.dataset.fotoRetry === '2') {
      img.onerror = null;
      img.src = (img.className && String(img.className).indexOf('vt-') >= 0) ? IMG_PLACEHOLDER_DARK : IMG_PLACEHOLDER_LOJA;
      return;
    }
    var src = img.getAttribute('src') || '';
    var n = parseInt(img.dataset.fotoRetry || '0', 10) + 1;
    img.dataset.fotoRetry = String(n);
    if (n === 1) {
      // tenta sem ?v=
      var limpo = src.split('?')[0];
      if (limpo && limpo !== src) { img.src = limpo; return; }
    }
    // 2ª tentativa: cache-bust
    var base = src.split('?')[0];
    img.src = base + (base.indexOf('?') >= 0 ? '&' : '?') + 'r=' + Date.now();
  } catch (e) {
    try { img.onerror = null; img.src = IMG_PLACEHOLDER_LOJA; } catch (e2) {}
  }
};
