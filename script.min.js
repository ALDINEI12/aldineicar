...
async function editarProduto(id) {
 ...
 ${(() => {
   ...
   const label = (s) => {
     if (s === 'Orçamento') return '📋 Orçamento';
     if (s === 'Em Execução') return '🔧 Em Execução';
     if (s === 'Concluído' || s === 'Pronto') return '✅ Concluído';
   ...
   if (st === 'Pago') {
     ...
     // Orçamento ou qualquer status inicial
     add(st === 'Orçamento' ? 'Orçamento' : st, label(st === 'Orçamento' ? 'Orçamento' : st));
...
function definirPeriodoRapido(tipo) {
 ...
 // marca período para o rótulo do card acompanhar (mês / ano / tudo)
...
/** Abre/fecha o bloco de gráficos + KPIs do resumo financeiro */
...
/** Atalho Resumo financeiro: toast com totais + abre o bloco detalhado */
...
/** Rótulo do período a partir das datas (hoje / ontem / 7 dias / mês / ano / personalizado) */
...
function atualizarDashMobileUI(dados, ini, fim) {
  try {
    // Faturamento do período (OS + serviços + vendas)
    var fat = (dados.totalFatOrc || 0) + (dados.totalServicos || 0) + (dados.totalVendas || 0);
