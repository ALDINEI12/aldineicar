...
function renderGraficoResumoFinanceiro(dados) {
  ...
  function draw(ChartLib) {
    var linhas = (dados && dados.linhas) ? dados.linhas.slice() : [];
    var labels = linhas.map(function(m) { return m.label || m.mes || ''; });
    var fat = linhas.map(function(m) { return (m.fatOrc || 0) + (m.servicos || 0) + (m.vendas || 0); });
    var luc = linhas.map(function(m) { return ((m.fatOrc || 0) + (m.servicos || 0) + (m.vendas || 0)) - (m.materiais || 0) - (m.gastos || 0); });
    ...
    graficoFinanceiroRfInstance = new ChartLib(canvas, {
      ...
      data: {
        labels: labels,
        datasets: [
          { label: 'Faturamento', data: fat, borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,.15)', tension: .3, fill: true },
          { label: 'Lucro', data: luc, borderColor: '#059669', backgroundColor: 'rgba(5,150,105,.1)', tension: .3, fill: true }
        ]
      },
      options: {
        responsive: true,
        ...
        scales: {
          ...
          y: { grid: { color: '#f1f5f9' }, ticks: { font: { size: 10 }, color: '#94a3b8', callback: function(v) { return v >= 1000 ? 'R$' + (v/1000).toFixed(1) + 'k' : 'R$' + v; } } }
        }
      }
    });
  }
