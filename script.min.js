...
function iniciarPollLojaPix(opts) {
  ...
  async function tick() {
    ...
    try {
      // 1) busca status MP por referência
      var pag = null;
      if (typeof buscarPagamentoMercadoPagoPorReferencia === 'function') {
        pag = await buscarPagamentoMercadoPagoPorReferencia(pend.external_reference || pend.txid);
      }
      if (pag && pag.status === 'approved') {
        var conf = await confirmarVendaLojaMP({
          office_id: pend.office_id,
          id_venda: pend.id_venda,
          external_reference: pend.external_reference || pend.txid,
          payment_id: pag.id
        });
        if (conf && conf.ok) {
          pararPollLojaPix();
