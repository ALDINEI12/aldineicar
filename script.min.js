...
var ADMIN_EMAILS_FRONT = ['SEU_EMAIL_ADMIN@gmail.com'];
...
function mostrarTelaNovaSenha() {
  ocultarAbasLoginSignup();
  var nova = document.getElementById('novaSenhaBox');
  if (nova) nova.style.display = 'block';
  var st = document.getElementById('novaSenhaStatus');
  if (st) st.textContent = 'Link validado. Defina sua nova senha.';
}
...
async function recuperarSenha() {
  ...
  try {
    ...
    if (redirectTo.indexOf('index.html') < 0) {
      // ensure points to app page
      redirectTo = window.location.origin + window.location.pathname;
    }
    redirectTo = redirectTo + (redirectTo.indexOf('?') >= 0 ? '&' : '?') + 'recuperar=1';
  ...
  async function onReady() {
    ...
    try {
      ...
      if (precisaRecuperar) {
        ...
        var ok = await processarLinkRecuperacao();
        if (ok) {
          if (st) st.textContent = 'Link validado. Defina sua nova senha abaixo.';
          mostrarTelaNovaSenha();
        } else {
          if (st) st.textContent = 'Não foi possível validar o link. Solicite um novo em Esqueci a senha.';
        }
      }
    } catch (e) {
      console.warn(e);
