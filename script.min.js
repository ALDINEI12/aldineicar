...
async function editarProduto(id) {
 ...
 if (typeof atualizarDashMobileUI === 'function') {
 try {
 var dadosFake = { totalFatOrc: 0, totalServicos: 0, totalVendas: 0 };
 // só atualiza contadores de status
 var lista = historico || [];
 ...
 var st = String(h.status || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
 if (st === 'pago' || st.indexOf('pago') === 0) { pago++; return; }
 if (st.indexOf('conclu') >= 0 || st.indexOf('pronto') >= 0) { ok++; return; }
 if (st.indexOf('execu') >= 0 || st.indexOf('andamento') >= 0) { serv++; return; }
 if (st.indexOf('cancel') >= 0) return;
 pend++;
 });
 var setN = function(id, n) { var e = document.getElementById(id); if (e) e.textContent = String(n); };
 setN('dashStPend', pend); setN('dashStServ', serv); setN('dashStOk', ok); setN('dashStPago', pago);
 } catch (e2) {}
 }
 }
}
  ...
  try {
    ...
    if (sub) {
      ...
      sub.textContent = 'Visão geral de ' + nome;
        ...
        // Status: só orçamentos + agenda (nunca vendas da loja)
    var lista = (typeof historico !== 'undefined' && Array.isArray(historico)) ? historico : [];
    var pend = 0, serv = 0, ok = 0, pago = 0;
    function ehVendaLoja(h) {
      var tipo = String(h.tipo_registro || '');
      if (tipo === 'VENDA_DIRETA_BALCAO' || tipo === 'LOJA_VIRTUAL') return true;
      if (h.pagamento && String(h.pagamento).indexOf('Mercado Pago') >= 0 && tipo.indexOf('VENDA') >= 0) return true;
      return false;
    }
    function ehOrcamentoOuAgenda(h) {
      if (ehVendaLoja(h)) return false;
      // agenda: tem objeto agendamento ou status típicos de agenda
      if (h.agendamento && (h.agendamento.data || h.agendamento.hora)) return true;
      var st0 = String(h.status || '');
      if (/^(Agendado|Confirmado)$/i.test(st0)) return true;
      // orçamento / OS do histórico (padrão)
      if (st0 === 'Orçamento' || st0 === 'Em Execução' || st0 === 'Pronto' || st0 === 'Pago') return true;
      // demais registros de serviço (cliente + veículo/serviço)
      if (h.cliente || h.veiculo || h.totalCobrado != null) return true;
      return false;
    }
    lista.forEach(function(h) {
      if (!h || !ehOrcamentoOuAgenda(h)) return;
      var st = String(h.status || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      // Pagos: só orçamentos/agenda com status Pago (não vendas)
      if (st === 'pago' || st.indexOf('pago') === 0) { pago++; return; }
      if (st.indexOf('conclu') >= 0 || st.indexOf('pronto') >= 0 || st.indexOf('finaliz') >= 0) { ok++; return; }
      if (st.indexOf('execu') >= 0 || st.indexOf('andamento') >= 0) { serv++; return; }
      if (st.indexOf('cancel') >= 0) return;
      // Pendentes: Orçamento, Agendado, Confirmado, etc.
      pend++;
    });
    var setN = function(id, n) { var e = document.getElementById(id); if (e) e.textContent = String(n); };
...
function dashAgendaIrParaDia(n) {
  window.__dashAgendaOffset = Math.max(0, Math.min(3, Number(n) || 0));
  if (typeof renderDashboard === 'function') renderDashboard();
  else if (typeof atualizarDashMobileUI === 'function') {
    try { atualizarDashMobileUI({ totalFatOrc:0, totalServicos:0, totalVendas:0 }, null, null); } catch(e) {}
  }
}
function dashAgendaMudarDia(delta) {
  var cur = typeof window.__dashAgendaOffset === 'number' ? window.__dashAgendaOffset : 0;
  dashAgendaIrParaDia(cur + (Number(delta) || 0));
}
