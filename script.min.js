  ...
  try {
    ...
    if (sub) {
      ...
      sub.textContent = 'Visão geral de ' + nome;
        ...
        // Status: orçamentos + agenda (ignora só vendas de produto da loja)
    ...
    lista.forEach(function(h) {
      ...
      if (st.indexOf('execu') >= 0 || st.indexOf('servico') >= 0 || st.indexOf('andamento') >= 0) { serv++; return; }
      // Cancelados não entram
      if (st.indexOf('cancel') >= 0) return;
      // Pendentes: Orçamento, Agendado, Confirmado, Aguardando, vazio
      if (
        st.indexOf('orcamento') >= 0 || st.indexOf('agend') >= 0 || st.indexOf('confirm') >= 0 ||
        st.indexOf('aguard') >= 0 || st.indexOf('pend') >= 0 || st === '' || st.indexOf('aberto') >= 0
      ) { pend++; return; }
      // demais status abertos contam como pendente
      pend++;
    });
    var setN = function(id, n) { var e = document.getElementById(id); if (e) e.textContent = String(n); };
    ...
    if (box) {
      ...
      lista.forEach(function(h) {
        if (!h) return;
        var isAg = (h.tipo_registro === 'AGENDAMENTO') || (h.tipo_servico && String(h.tipo_servico).indexOf('AGEND') >= 0)
          || (h.veiculo && h.veiculo.tipo_servico && String(h.veiculo.tipo_servico).toLowerCase().indexOf('agend') >= 0)
          || h.hora || h.horario;
        if (!isAg && !(h.data && h.cliente && (h.status || '').toLowerCase().indexOf('agend') >= 0)) {
          // também pega itens com data de hoje e hora
          if (!(h.hora || h.horario)) return;
        }
        var dataH = String(h.data || h.data_agendamento || '').split(',')[0].trim();
        if (dataH && dataH.indexOf(hojeStr) < 0 && dataH !== hojeStr) {
          // tenta match parcial dd/mm
          var parts = hojeStr.split('/');
          if (!(dataH.indexOf(parts[0] + '/' + parts[1]) >= 0)) return;
        }
        if (!dataH) return;
        var nomeCli = '';
        if (h.cliente) {
        ...
        var servico = h.servico_nome || h.produto_nome || (h.veiculo && (h.veiculo.modelo || h.veiculo.tipo_servico)) || 'Serviço';
        var hora = h.hora || h.horario || '--:--';
        itens.push({ hora: hora, nome: nomeCli, servico: String(servico).slice(0, 40) });
      });
      // fallback: pega da agenda se houver estrutura diferente
      if (!itens.length && typeof window.__histAgendaCliente !== 'undefined' && Array.isArray(window.__histAgendaCliente)) {
        window.__histAgendaCliente.forEach(function(h) {
          var dataH = String(h.data || '').split(',')[0].trim();
          if (dataH.indexOf(hojeStr.split('/').slice(0,2).join('/')) < 0) return;
          var nomeCli = (h.cliente && h.cliente.nome) || h.cliente_nome || 'Cliente';
          itens.push({ hora: h.hora || h.horario || '--:--', nome: nomeCli, servico: h.servico_nome || 'Serviço' });
        });
      }
      itens = itens.slice(0, 6);
      if (!itens.length) {
        box.innerHTML = '<p class="dash-empty">Nenhum agendamento para hoje.</p>';
      } else {
        box.innerHTML = itens.map(function(it) {
          return '<div class="dash-ag-item"><div class="dash-ag-hora">' + it.hora + '</div>'
            ...
            + '<div class="dash-ag-serv">' + it.servico + '</div></div></div>';
        }).join('');
      }
    }
...
function renderListaDiaSelecionado(todos) {
 ...
 + '<button onclick="alterarStatusAgendamento(' + idx + ',\'Em Execução\')" class="agenda-btn agenda-btn-exec">🔧 Em execução</button>'
 ...
 + '<button onclick="alterarStatusAgendamento(' + idx + ',\'Pago\')" class="agenda-btn agenda-btn-ok">💰 Pago</button>'
 + '<button onclick="alterarStatusAgendamento(' + idx + ',\'Cancelado\')" class="agenda-btn agenda-btn-cancel">✕ Cancelar</button>';
 }
