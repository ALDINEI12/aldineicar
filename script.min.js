...
async function recuperarSenha() {
  var emailEl = document.getElementById('loginEmail') || document.getElementById('recuperarEmail');
  var email = (emailEl && emailEl.value || '').trim();
  if (!email) {
    alert('Digite seu e-mail no campo acima e toque em Esqueci a senha.');
    return;
  }
  try {
    var redirectTo = window.location.origin + window.location.pathname + '?recuperar=1';
    var res = await supabaseClient.auth.resetPasswordForEmail(email, { redirectTo: redirectTo });
    if (res.error) throw res.error;
    alert('Enviamos um link de recuperação para ' + email + '.\nAbra o e-mail e defina uma nova senha.');
  } catch (e) {
    console.error(e);
...
async function salvarNovaSenhaRecuperacao() {
  ...
  try {
    ...
    if (res.error) throw res.error;
    alert('Senha atualizada! Você já pode entrar.');
    var box = document.getElementById('recuperarSenhaBox');
...
function ativarMenuAdminSePermitido(email) {
  ...
  var ok = isEmailAdmin(email);
  if (btn) btn.style.display = ok ? '' : 'none';
  if (side) side.style.display = ok ? '' : 'none';
}
