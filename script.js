...
async function editarProduto(id) {
    ...
    async function salvarDadosOficina() {
        ...
        var waEl = document.getElementById('oficinaWhatsapp');
        var igEl = document.getElementById('oficinaInstagram');
        var mpEl = document.getElementById('oficinaMaps');
        if (waEl) dadosOficina.whatsapp = (waEl.value || '').trim();
        if (igEl) dadosOficina.instagram = (igEl.value || '').trim();
        if (mpEl) dadosOficina.maps_url = (mpEl.value || '').trim(); 
...
async function enviarOrcamentoCliente() {
    ...
    try {
        ...
        ['cliOrcNome','cliOrcTelefone','cliOrcVeiculo','cliOrcDescricao','cliOrcData','cliOrcHora','cliOrcServico'].forEach(id => {
...
function normalizarInstagramUser(v){ return String(v||'').trim().replace(/^@/,'').replace(/https?:\/\/(www\.)?instagram\.com\//i,'').replace(/\/$/,''); }
...
function renderServicosVitrinePublica(lista){
  ...
  if(!items.length){ box.innerHTML='<div class="vt-empty"><div class="vt-empty-icon">🔧</div><p>Serviços em atualização. Use Agendar e descreva o que precisa.</p></div>'; return; }
...
(function(){
  var orig=typeof mostrarAba==='function'?mostrarAba:null;
  if(!orig||orig.__servAdminPatched) return;
  window.mostrarAba=function(aba){ var r=orig.apply(this,arguments); if(aba==='loja') renderServicosVitrineAdmin(); return r; };
  window.mostrarAba.__servAdminPatched=true;
})();
