...
function gerarPayloadPix({ chave, nome, cidade, valor, txid }) {
    ...
    payload += pixTLV('54', Number(valor).toFixed(2));
    payload += pixTLV('58', 'BR');
    payload += pixTLV('59', nomeLimpo);
    payload += pixTLV('60', cidadeLimpa);
    payload += pixTLV('62', pixTLV('05', (txid || '***').substring(0, 25)));
    payload += '6304';
      ...
      <div style="background:#1c1c1e;padding:24px;border-radius:12px;width:90%;max-width:440px;max-height:92vh;overflow-y:auto;border:1px solid #2c2c2e;color:#fff;font-family:sans-serif;">
        <div id="etapaDadosCompra">
          ...
          <div style="display:flex;gap:10px;justify-content:flex-end;">
            ...
            <button type="button" onclick="gerarPixEMostrarQR()" style="background:#22c55e;color:#fff;border:none;padding:10px 20px;border-radius:8px;font-weight:700;cursor:pointer;">Gerar PIX →</button>
          </div>
        </div>

        <div id="etapaPixCompra" style="display:none;">
          <h3 style="margin:0 0 8px;font-size:18px;font-weight:700;">Pagamento via PIX</h3>
          <p style="font-size:13px;color:#a1a1aa;margin:0 0 12px;">Escaneie o QR Code ou copie o código. Depois confirme o pagamento.</p>
          <div style="text-align:center;background:#09090b;border-radius:12px;padding:16px;border:1px solid #27272a;margin-bottom:14px;">
            ...
            <div id="pixQrBox" style="display:flex;justify-content:center;margin-bottom:12px;">
          ...
          <p style="font-size:11px;color:#a1a1aa;margin:0 0 14px;line-height:1.4;">Chave PIX (CPF Nubank): <b style="color:#fff;">116.843.885-38</b></p>
          <div style="display:flex;gap:10px;flex-wrap:wrap;">
            <button type="button" onclick="voltarEtapaDadosCompra()" style="flex:1;background:#3a3a3c;color:#fff;border:none;padding:12px;border-radius:8px;font-weight:600;cursor:pointer;">← Voltar</button>
            <button type="button" onclick="confirmarPagamentoPixEFinalizar()" style="flex:2;background:#22c55e;color:#fff;border:none;padding:12px;border-radius:8px;font-weight:700;cursor:pointer;">✓ Já paguei — Confirmar</button>
          </div>
        </div>
      </div>
    `;
...
function desenharQrNoCanvas(payload) {
    ...
    if (typeof QRCode !== 'undefined' && canvas) {
        try {
            QRCode.toCanvas(canvas, payload, {
                ...
                if (err) {
                    console.error('QRCode lib error', err);
                    fallbackQrImagem(payload, box, canvas);
                }
            });
            return;
        } catch (e) {
    ...
    // 2) Fallback imagem online
    fallbackQrImagem(payload, box, canvas);
}

function fallbackQrImagem(payload, box, canvas) {
...
async function gerarPixEMostrarQR() {
    try {
        ...
        const etapaDados = document.getElementById('etapaDadosCompra');
        const etapaPix = document.getElementById('etapaPixCompra');
        if (etapaDados) etapaDados.style.display = 'none';
        if (etapaPix) etapaPix.style.display = 'block';

        const valorEl = document.getElementById('pixValorGrande');
        if (valorEl) valorEl.innerText = 'R$ ' + total.toFixed(2);

        const ta = document.getElementById('pixCopiaCola');
        if (ta) ta.value = payload;

        // Garante o canvas existe após mostrar a etapa
        setTimeout(function() {
            desenharQrNoCanvas(payload);
        }, 50);
...
async function confirmarPagamentoPixEFinalizar() {
    ...
    const ok = confirm('Confirma que o PIX de R$ ' + dadosCompraPendente.total.toFixed(2) + ' já foi pago?');
    if (!ok) return;
...
async function processarBaixaEstoqueEPix() {
    if (!dadosCompraPendente) {
        alert('Conclua o pagamento PIX antes de finalizar.');
        return;
    }
    ...
    try {
        ...
        const { error: erroUpdate } = await supabaseClient
            .from('user_data')
            .update({ historico: listaHistoricoGeral, updated_at: new Date() })
            .eq('user_id', idDonoVenda);
        if (erroUpdate) throw erroUpdate;

        // Baixa estoque produtos_loja
        try {
            ...
            if (prod) {
                ...
                await supabaseClient.from('produtos_loja').update({ estoque: novoEstoque }).eq('id', produtoSelecionadoParaCompra);
            }
        } catch (eEst) {
            console.warn('Estoque:', eEst);
        }
